...
| 全域狀態列 | **不做**（本倉庫沒有對應的狀態來源） |

## 版面

- **不新增全域狀態列。** 截圖上方的「未開始」是參考版自己的東西，本倉庫沒有對應物：
  只有 `MultiplayerSession.RunStatus`，它是多人專用、閒置時為空字串，且已經印在
  多人視窗 `Draw()` 的第一行。維持原位，不抽到分頁列上方。
- **練習**分頁：維持現在的兩欄表格與 `<` 收合，內容不動。
- **精簡模擬控制維持現狀，不加分頁列。** `Draw()` 開頭在「位於模擬場地且啟用精簡控制」
  時會畫一個場景下拉與精簡內容後直接 return；該路徑不變，分頁列只出現在完整面板。
