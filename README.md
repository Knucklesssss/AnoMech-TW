...
在旅館或住宅室內單人練習高難副本機制。插件會在你的客戶端生出假隊友和假 boss，讓它們照真實時間軸跑機制——
全部在本機模擬，不會送任何東西到伺服器。

進旅館、個人／公會住宅、個人房間或公寓房間後打 `/anomech`（或 `/ano`）開啟，房內有其他玩家也可使用。不開放屋外住宅區、工房與公寓大廳。

### 0.3.16.7 更新

- P2–P5 新增「B站莫古力」打法選項，保留原有打法。
- P2.5 新增系統自動標記及自身標記指定；P3 小電視新增固定式／鏡像換邊式。
- 修正莫古力 P5 一運綠線靠蟑螂、藍線靠光頭的站位與手臂引導；對照攻略調整一傳路線。
- 修正二傳標記站位、旋轉雷射跑位及三傳面向小電視的左右優先級。
- 新增住宅室內支援、載入失敗保護；切換不同副本前須先離開模擬。
- 自動測試與編譯通過，使用者已回報實測正常。完整職業技能循環另行開發，不包含在此版。
...
- P3「Hello, World」是從單次錄影重建的，Bot 走位依攻略手寫，尚未逐項驗證完畢；
  該場景的傷害與失誤判定尚未實作（站錯不會死）。「螢幕砲」則有承傷判定。
