...
public sealed class TopP5SigmaTuuuflessAi : TopP5SigmaAi
{
    ...
    // Gap i is the point between waymark i and waymark i+1 — where a player who
    // shifted one eighth clockwise off their own waymark lands after the knockback.
    ...
    // Mid glitch spawns its six towers on the gaps, so the tower underfoot is the one
    // to ignore: each gap reaches left or right, taking the two-player tower first.
    ...
    // Far glitch spawns its five towers on the waymarks themselves, so each gap simply
    // looks at the waymark either side of it, again preferring the two-player tower.
