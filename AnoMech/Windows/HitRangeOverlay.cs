...
public sealed class HitRangeOverlay : Window
{
        ...
        foreach (var range in ranges)
        {
            ...
            switch (range.Shape)
            {
                ...
                case HitRangeShape.Cone:
                    Cone(draw, coordinates, range, colour, backing);
                    break;
                ...
                case HitRangeShape.Cross:
                    ...
                    Rect(draw, coordinates, range.Origin, range.Rotation + MathF.PI / 2f,
                        range.A, -range.B, range.B, colour, backing);
                    break;
            }
        }
    }
    ...
    private void Caption(ImDrawListPtr draw, Coordinates coordinates, HitRange range, uint colour, uint backing)
    {
        ...
        {
            HitRangeShape.Circle => $"半徑={range.A:0.#}",
            HitRangeShape.Ring => $"半徑={range.A:0.#}~{range.B:0.#}",
            HitRangeShape.Cone => $"{range.A * 2f * 180f / MathF.PI:0}° 長={range.B:0.#}",
            HitRangeShape.Rect => $"寬={range.A * 2f:0.#} 長={range.B:0.#}",
            HitRangeShape.Cross => $"寬={range.A * 2f:0.#} 長={range.B * 2f:0.#}",
            _ => "",
        };
        ...
        uint colour, uint backing)
    {
        ...
        Polyline(draw, points[..count], colour, backing, closed: count == Segments);
    }
        ...
        uint colour, uint backing)
    {
        ...
        Polyline(draw, points[..count], colour, backing, closed: true);
    }

    private void Rect(ImDrawListPtr draw, Coordinates coordinates, Vector3 origin, float rotation,
        float halfWidth, float back, float front, uint colour, uint backing)
    {
        ...
        foreach (var corner in corners)
            if (Project(coordinates, corner, out var screen))
                points[count++] = screen;
        Polyline(draw, points[..count], colour, backing, closed: count == 4);
    }

    private static void Polyline(ImDrawListPtr draw, ReadOnlySpan<Vector2> points,
        uint colour, uint backing, bool closed)
    {
