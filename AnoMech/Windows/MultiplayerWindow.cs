...
internal sealed class MultiplayerWindow : Window, IDisposable
{
    ...
    private static readonly string ConnectionExplanation =
        ...
        "固定 IP：\n" +
        "・不需要申請固定 IP。一般家用網路的外部位址偶爾會變（重開數據機後常見），插件每次開房都會重新查詢。\n" +
        "・不管有沒有固定 IP，每次開房的邀請碼都不一樣（包含新的房間代碼），請傳最新的給朋友。\n" +
        "・真正要固定的是「這台電腦的區網 IP」（192.168.x.x）。它變了，路由器會轉發到錯的電腦，朋友就連不進來。\n" +
        "  請在路由器的「DHCP 保留」或「固定 IP 分配」把這台電腦的區網 IP 綁定下來，路由器規則就不用重設。";
