...
// this runtime owns the shared practice reservation and projects a temporary native gauge only while this runtime exists.
internal sealed unsafe class PracticeLimitBreakRuntime : IDisposable
{
    ...
    private readonly PracticeLimitBreakGauge gauge = new();
    private readonly PracticeLimitBreakNativeCast nativeCast = new();
    ...
    internal uint CastingAction => active is { Caster: SimPlayer } cast ? cast.ActionId : 0;

    internal void AfterNativeUpdate()
    {
    ...
    internal bool IsRecovering(PartyRole role) => !disposed && (uint)role < 8 && recovering[(int)role] != null;
        ...
        SimCharacter? target = null)
    {
        ...
        if (location is { } point && (!float.IsFinite(point.X) || !float.IsFinite(point.Y) || !float.IsFinite(point.Z)))
            return false;
    ...
    private static void SyncPlayerLock(PendingCast pending)
    {
    ...
    private void ApplyTankStatus(PendingCast pending)
    {
        ...
        foreach (var member in world.Party.ActiveMembers())
        {
            if (radius > 0f && Vector3.DistanceSquared(pending.Caster.Position, member.Position) > radiusSquared)
                continue;
            member.AddStatusParam(statusId, 0, duration, sourceObject);
        }
    }
