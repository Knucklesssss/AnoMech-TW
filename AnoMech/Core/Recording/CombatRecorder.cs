...
internal sealed unsafe partial class CombatRecorder : IDisposable
{
        ...
        byte sequence, byte unknown, byte* buffer, ulong length)
    {
        ...
        catch (Exception ex)
        {
            ...
            Plugin.Log.Information($"[錄影] dirdata 出錯（已忽略）：{ex.Message}");
        }
        finally
        {
    ...
    {
        if (Plugin.GameInstance.World.Map.IsInInstance || Plugin.ObjectTable.LocalPlayer == null)
        {
            LastError = "請登入並離開模擬場景後再開始錄製。";
            return;
        }
        ...
        lock (recordingGate)
        {
            ...
            {
                Plugin.Log.Information($"[錄影] 已在錄同一個 territory（{territoryNow}），沿用（{reason}）");
                return;
            }
            ...
            try
            {
                ...
                {
                    Plugin.Log.Information($"[錄影] 起錄標頭寫入失敗：{CurrentPath}");
                    return;
                }

                Plugin.Log.Information($"[錄影] 開始：{CurrentPath}（{reason}）");
            }
            catch (Exception ex)
            {
                if (opened != null)
                ...
                Plugin.Log.Information($"[錄影] 開始失敗：{ex.Message}");
            }
        }
    }
    ...
    private void StopLocked()
    {
        ...
        if (controlCat35Count > 0 && tetherCount == 0)
            Plugin.Log.Information($"[錄影] ⚠ tether hook 掛上但整場沒觸發（cat35={controlCat35Count}）"
        ...
        if (!flushed || !closed)
        {
            Plugin.Log.Information($"[錄影] ⚠ 封口失敗（{CurrentPath}）：{LastError}");
            return;
        }
        ...
        {
            RecordExternalErrorLocked(ex);
            Plugin.Log.Information($"[錄影] 封口發布失敗，保留未封口原始檔：{CurrentPath}");
            return;
        }

        Plugin.Log.Information($"[錄影] 結束：{CurrentPath}，共 {eventsBeforeStop} 筆，完整=true");
    }
    ...
    private void RecordExternalErrorLocked(Exception ex)
    {
        ...
        Plugin.Log.Information($"[錄影] I/O 錯誤：{LastError}");
    }
    ...
    private void FailWriterLocked(RecordingWriter failed)
    {
        ...
        failed.Close();
        SyncWriterDiagnosticsLocked(failed);
        Plugin.Log.Information($"[錄影] 寫入失敗，已停止錄影（{CurrentPath}）：{LastError}");
        ResetCaptureStateLocked();
    }
    ...
    private unsafe void PollWeather()
    {
        ...
        lastWeather = w;
        Write("weather", $"\"id\":{w}");
        Plugin.Log.Information($"[錄影] 天氣 → {w}");
    }
    ...
    private void ControlDetour(uint entityId, uint category, uint p1, uint p2, uint p3, uint p4,
                               uint p5, uint p6, uint p7, uint p8, GameObjectId targetId, bool p10)
    {
        try
        {
            if (writer != null)
            {
                var sequence = WriteHook("control", "control",
                  ...
                  + $"\"target\":{(ulong)targetId},\"p10\":{(p10 ? "true" : "false")}");
        ...
        try
        {
            if (writer != null && packet != null)
                ...
                WriteHook("spawn", "spawnobj",
                  ...
                  + $"\"eventId\":{packet->EventId.Id},\"gimmickId\":{packet->GimmickId},"
