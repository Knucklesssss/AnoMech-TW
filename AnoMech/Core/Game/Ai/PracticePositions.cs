...
public sealed class PracticePositions
{
    ...
    private readonly Func<bool>? singlePlayer;
    ...
    public PracticePositions(IPracticePositionStore store, Func<bool>? singlePlayer = null)
    {
    ...
    public bool IsActive => active is not null && !MultiplayerContext.InRun && (singlePlayer?.Invoke() ?? true);
    ...
    /// </summary>
    public void Begin(string scenarioKey, string scenarioName, string stratName, string scenarioType)
    {
        End();
        if (MultiplayerContext.InRun || singlePlayer?.Invoke() == false || scenarioType is
            "AnoMech.Scenarios.Top.P3HelloWorld.TopP3HelloWorldScenario" or
            "AnoMech.Scenarios.Top.P3Monitors.TopP3MonitorsScenario") return;
        EnsureLoaded();
        ...
        IReadOnlyList<Vector2?>? reference = null)
    {
        ...
        if (!IsActive || step.Index < 0) return originalSlots;
    ...
    {
        if (!IsActive) return Fail("尚未開始 scenario，無法還原步驟。");
    ...
    {
        if (!IsActive) return Fail("尚未開始 scenario，無法建立軌跡區間。");
    ...
    public bool TryUpdateInterval(
        ...
        Vector2? endPosition = null)
    {
        if (!IsActive) return Fail("尚未開始 scenario，無法更新軌跡區間。");
        if (!float.IsFinite(startTime) || !float.IsFinite(endTime) || endTime < startTime)
            return Fail("軌跡區間必須是有效的起始／結束秒數，且結束不得早於起始。");
        if (slot is < 0 or >= PracticePositionLogic.PartySlotCount)
            return Fail("角色欄位超出 MT/ST/H1/H2/D1-D4 範圍。");
    ...
    {
        if (!IsActive) return Fail("尚未開始 scenario，無法刪除軌跡區間。");
        return Commit(document =>
        {
    ...
    {
        if (!IsActive) return Fail("尚未開始 scenario，無法還原原始路線。");
        return Commit(document =>
        {
